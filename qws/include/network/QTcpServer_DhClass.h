...
class DhQTcpServer : public DynamicQHandler, public QTcpServer
{
private: 
...
public:
  ...
  explicit DhQTcpServer() : DynamicQHandler(), QTcpServer() {};
  explicit DhQTcpServer(QObject* x1) : DynamicQHandler(), QTcpServer(x1) {};
  bool hasPendingConnections() const;
  bool DhhasPendingConnections() const;
  bool DvhhasPendingConnections() const;
  void incomingConnection(int x1);
  void DhincomingConnection(int x1);
  void DvhincomingConnection(int x1);
  QTcpSocket* nextPendingConnection();
  QTcpSocket* DhnextPendingConnection();
  QTcpSocket* DvhnextPendingConnection();
  ~DhQTcpServer(){}
