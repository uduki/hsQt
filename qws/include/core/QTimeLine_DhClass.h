...
class DhQTimeLine : public DynamicQHandler, public QTimeLine
{
private: 
...
public:
  ...
  explicit DhQTimeLine() : DynamicQHandler(), QTimeLine() {};
  explicit DhQTimeLine(int x1) : DynamicQHandler(), QTimeLine(x1) {};
  explicit DhQTimeLine(int x1, QObject* x2) : DynamicQHandler(), QTimeLine(x1, x2) {};
  void timerEvent(QTimerEvent* x1);
  void DhtimerEvent(QTimerEvent* x1);
  void DvhtimerEvent(QTimerEvent* x1);
  qreal valueForTime(int x1) const;
  qreal DhvalueForTime(int x1) const;
  qreal DvhvalueForTime(int x1) const;
  ~DhQTimeLine(){}
