...
main
  = do
    ...
    loader <- qUiLoader ()
    setHandler loader "(QWidget*)createWidget(const QString&,QWidget*,const QString&)" myCreateWidget
...
myCreateWidget :: QUiLoader () -> String -> QWidget a -> String -> IO (QWidget ())
myCreateWidget this cnm pr nm
  = do
    if (cnm == "QPushButton")
     then do
      tb <- qPushButton pr
      setHandler tb "(QSize)sizeHint()" mbs
      setObjectName tb nm
      return (objectCast tb)
     else do
      createWidget_h this (cnm, pr, nm)
...
ui_init ui
  = do
    ...
    setStyleChildren ui =<< arthurStyle
